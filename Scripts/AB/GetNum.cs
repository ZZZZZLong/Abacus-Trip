...
public class GetNum : MonoBehaviour
{
    ...
    void GetSum(int j)
    {
        ...
        {
            float atotal = int.Parse(GetObjName(i, ObjButtomImages)) + (int.Parse(GetObjName(i, ObjTopImages))) * 5;//下珠+上珠*5等于这一个位数的值
        ...
        {
            Debug.Log("循环");
            float atotal2 = int.Parse(GetObjName(i - 1, ObjButtomImages)) + (int.Parse(GetObjName(i - 1, ObjTopImages)) * 5); // 下珠+上珠*5等于这一个位数的值
            double power = Math.Pow(10, i - j - 1); // 小数部分的权重为 10 的负 i 次方
            atotal2 *= (float)power; // 小数部分的值
            total += atotal2;
        }
